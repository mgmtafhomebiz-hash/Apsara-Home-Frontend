    ...
    <div className="rounded-3xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
      <div className="flex items-center justify-between gap-3 border-b border-gray-200 px-6 py-5 dark:border-gray-700">
        <div>
          ...
          <h2 className="mt-1 text-lg font-bold text-gray-900 dark:text-gray-100">Recent Admin Product Activity</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">Latest product actions across admin accounts.</p>
        </div>
        <Link
          href="/admin/products"
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="h-16 animate-pulse rounded-2xl bg-gray-200 dark:bg-gray-800/80" />
            ))}
          </div>
        ) : logs.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-5 py-10 text-center dark:border-gray-700 dark:bg-gray-800/60">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-100">No admin product activity yet</p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">New product uploads, edits, deletes, and failures will appear here.</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="max-h-[30rem] space-y-3 overflow-y-auto pr-1">
              {logs.map((log) => (
                <div key={log.id} className="rounded-2xl border border-gray-200 px-4 py-3 dark:border-gray-700 dark:bg-gray-800/60">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <p className="truncate text-sm font-semibold text-gray-800 dark:text-gray-100">{log.productName}</p>
            ...
            <div className="flex flex-col gap-3 border-t border-gray-200 pt-4 dark:border-gray-700 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-gray-500 dark:text-gray-300">
                Page <span className="font-semibold text-gray-700 dark:text-gray-100">{currentPage}</span> of <span className="font-semibold text-gray-700 dark:text-gray-100">{lastPage}</span>
