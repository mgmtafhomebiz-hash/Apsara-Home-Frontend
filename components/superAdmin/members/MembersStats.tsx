...
const MembersStats = ({ stats, onTotalEarningsClick }: MembersStatsProps) => {
  const cards = [
    {
      label: 'Total Members',
    ...
    {
      key: 'total_earnings',
      label: 'Total Earnings (All Members)',
          ...
          {card.key === 'total_earnings' ? (
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <div className={`${card.iconBg} h-8 w-8 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform duration-300`}>
                  {card.icon}
                </div>
              </div>
              <p className={`text-lg font-bold ${card.color}`}>{card.value}</p>
              <p className="mt-1 text-[11px] text-slate-400">Click to trace earners in current list</p>
            </button>
          ) : (
            <>
              <div className="flex items-start justify-between gap-2 mb-3">
